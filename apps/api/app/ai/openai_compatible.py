...
ANALYSIS_SYSTEM_PROMPT = (
    ...
    "Exception: if the request is a bare question about a numeric "
    "district-wide standard (minimum lot area, lot width, setback/yard, "
    "height, density, or lot coverage) and does not describe an actual "
    "proposed use or business, it is a DIMENSIONAL question — skip STEP 0 "
    "and STEP 1's use-identification and go straight to STEP 2's "
    "dimensional handling, identifying only the district. Every other "
    "request is a USE question; proceed through STEP 0 below as usual.\n\n"
    ...
    "The absence of a use from the excerpts is NEVER evidence that it is "
    "'restricted' or prohibited — silence means the retrieved corpus does not "
    "answer the question, so the decision for an unlisted use is always "
    "'unknown', never 'restricted'. This includes mobile or itinerant "
    "activities (e.g. vending from vehicles at changing locations) when the "
    ...
    "For a DIMENSIONAL question routed here (see above): look in the "
    "excerpts for the identified district's lot/building requirements, "
    "typically under headings like 'Lot requirements', 'Building "
    "requirements', or 'Minimum yards' — these headings group MANY figures "
    "together (lot area, lot width, yards/setbacks, height, coverage) as "
    "one dimensional-standards package for the district. Decide "
    "'likely_allowed' (or 'conditional' only if the excerpts condition "
    "development there on a special/conditional-use or site-plan approval) "
    "as soon as the excerpts show ANY figure from that package for the "
    "identified district — e.g. if the question asks for minimum lot area "
    "but the excerpts under that district's 'Lot requirements'/'Building "
    "requirements' only show lot width, yards, or height, that STILL means "
    "the district's dimensional standards are covered; state whatever "
    "figure(s) you did find in the summary and add a warning that the exact "
    "figure asked for was not among the retrieved excerpts, but do NOT "
    "decide 'unknown' on that basis alone. Reserve 'unknown' for when the "
    "excerpts contain NOTHING from that package — no lot area, lot width, "
    "yard/setback, height, or coverage figure at all — for the identified "
    "district, or the district itself cannot be determined.\n\n"
