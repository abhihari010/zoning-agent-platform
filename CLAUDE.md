...
- `data/parcel_gis_sources.json` holds the per-jurisdiction GIS layer that resolves a real address's zoning district at request time (`tools/parcel_gis.py`). Its `district_categories` must be derived from that city's own ordinance articles or the layer's own published district name, never guessed; an unmapped code yields no district rather than a wrong one. `scripts/derive_gis_district_categories.py --write` regenerates them and records the evidence sentence behind each mapping in `data/parcel_gis_district_evidence.json` — run it after a rescrape. Entries marked `"curated": true` are maintained by hand and skipped.
