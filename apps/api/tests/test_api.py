...
def test_jurisdiction_coverage_and_request_flow(monkeypatch):
    ...
    assert any(item["jurisdiction_id"] == "roanoke-va" and item["coverage_status"] == "public_supported" for item in coverage)
    assert any(item["jurisdiction_id"] == "goodlettsville-tn" and item["coverage_status"] == "source_indexed" for item in coverage)
